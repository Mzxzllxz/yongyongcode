...
function run(code, input) {
	...
	if (statements[0] !== '어떻게' || !statements.slice(-1)[0].startsWith('이 동물이름이냐')) {
	...
	function execute(statement) {
		if (statement.includes('거북') && statement.includes('?')) { // IF GOTO
		...
		if (statement.includes('김')) {
			const variablePointer = statement.split('김')[0].split('어').length;
			const setteeValue = evaluate(statement.split('김')[1]);
			variables[variablePointer] = setteeValue;
		}

		if (statement.includes('덕') && statement[statement.length - 1] === '!') {
			printOut(String(evaluate(statement.slice(1, -1))));
		}

		if (statement.includes('덕') && statement[statement.length - 1] === 'ㅋ') {
			if (statement === '덕ㅋ') printOut('\n');
			printOut(stringify(evaluate(statement.slice(1, -1))));
		}

		if (statement.includes('용')) {
			pointer = evaluate(statement.split('용')[1]) - 1;
		}
	...
	function parse() {
		if (statements[pointer].startsWith('이 동물이름이냐')) stop();
		if (STOP) {
			STOP = false;
			stop();
		}
